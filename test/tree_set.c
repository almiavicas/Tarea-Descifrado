...
int main()
{
	...
	int inserted;
	inserted = insert(c, person1->age, person1);
	if (!inserted) {
	...
	person_info(person2);
	p = (struct person *) get(c, 15);
	printf("%s, %d\n", p->name, p->age);
	printf("%p\n", p);
	printf("getting person 3. its real info is:\n");
	person_info(person3);
	p = (struct person *) get(c, 20);
	printf("%s, %d\n", p->name, p->age);
	printf("%p\n", p);
	return 0;
}